...
public class CountryDoesNotExistAspect {
    ...
    @AfterThrowing(pointcut = "execution(* com.pluralsight.springaop.flightsmanagement.dao.PassengerDaoImpl.insert(..))", throwing = "ex")
    public void log(CountryDoesNotExistException ex) {
        logger.severe("Attempt to insert a passenger with a non existent country: " + ex.getCountryCode());
    }
